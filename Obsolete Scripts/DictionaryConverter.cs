...
public class DictionaryConverter : MonoBehaviour
{
    ...
    public Dictionary<string, string> sentences_Sample
    {
        get { return sentences_Sample; }
        set { sentences_Sample = value; }
    }
    public Dictionary<string, string> responses_Sample
    {
        get { return responses_Sample; }
        set { responses_Sample = value; }
    }
    public Dictionary<string, string> btnText_Sample
    {
        get { return btnText_Sample; }
        set { btnText_Sample = value; }
    }
    ...
    {
        //Provided example
        Dialogue sampleDialogue = JsonUtility.FromJson<Dialogue>(File.ReadAllText(Application.streamingAssetsPath + "/SampleDialogue_dict.json"));
        ...
        string[] bTags_Sample = siebelDialogue.tags_bText;
        string[] buttonTitle_Siebel = siebelDialogue.btnText;

        //Constructing Unity dictionaries with information from json file above

        //Compiling dictionary for sentences
        sentences_Sample = compileDictionary(sTags_Sample, sentences_Sample);
        //Compiling dictionary for responses
        responses_Sample = compileDictionary(rTags_Sample, responses_Sample);
        //Compiling dictionary for button entries
        btnText_Sample = compileBtnDictionary(bTTags_Siebel, bText_Siebel);
	...
	private Dictionary<string, string> compileDictionary(string[] tags, string[] text, Dictionary<string, string> DictDump){
